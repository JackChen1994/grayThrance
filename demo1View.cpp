...
{
	m_LeftEyeLeftCorner.x=122;
	m_LeftEyeLeftCorner.y=258;
	m_LeftEyeRightCorner.x=155;
	m_LeftEyeRightCorner.y=258;
	m_LeftEye.x=124;
	m_LeftEye.y=181;
	m_RightEyeLeftCorner.x=207;
	m_RightEyeLeftCorner.y=258;
	m_RightEyeRightCorner.x=247;
	m_RightEyeRightCorner.y=260;
	m_RightEye.x=185;
	m_RightEye.y=185;

	CDemo1Doc *pDoc=GetDocument();
	ImageDib *pdib1=pDoc->GetPDib();
...
void CDemo1View::Onlight() 
{
	...
	Light simcalculation(pDib->GetDimensions(),pDib->m_nBitCount,
		pDib->m_lpColorTable, pDib->m_pImgData,NULL);
...
void CDemo1View::OnLbpc() 
{
	...
	LBP graytrans(size,pDib->m_nBitCount, pDib->m_lpColorTable, pDib->m_pImgData);
